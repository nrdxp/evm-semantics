...
tests/specs/%.provex: tests/specs/% tests/specs/$$(firstword $$(subst /, ,$$*))/$$(KPROVE_FILE)/$(TEST_SYMBOLIC_BACKEND)/$$(KPROVE_FILE)-kompiled/timestamp
	$(KEVM) prove $< $(TEST_OPTIONS) --backend $(TEST_SYMBOLIC_BACKEND) --format-failures $(KPROVE_OPTS)        \
	    --provex --directory tests/specs/$(firstword $(subst /, ,$*))/$(KPROVE_FILE)/$(TEST_SYMBOLIC_BACKEND)
...
tests/specs/summarize/%-spec.k: tests/specs/summarize/%.bytecode $(KEVM_BIN)/kevm $(KEVM_LIB)/summarize/counterexample-spec.tmpl.k
	$(KEVM) counterexample-spec $< > $@
